...
class Reputation_command(commands.Cog):
    ...
    async def meme(self, ctx):
        ...
        say = "Ваша репутация:" + str(data[ins][str(ctx.message.author)])
        await ctx.channel.send(say)
        say = "Ваш уровень:" + str(data[ins][str(ctx.message.author)] // 5)
        await ctx.channel.send(say)
        f.close()
...
class rep_eveent(commands.Cog):
    ...
    async def on_message(self, message):
        if str(message.author) == self.bot:
            print("No hs")
            return
        print(1)
        try:
        ...
        for i in range(len(data)):
            for key in data[i]:
                f.write(str(key + ":" + str(data[i][key]) + "\n"))
        print(ins)
        say = "Ваша репутация:" + str(data[ins][str(message.author)])

        if data[ins][str(message.author)] % 5 == 0:
            say = "Поздравляем, вы достигли уровня " + str(data[ins][str(message.author)] // 5)
